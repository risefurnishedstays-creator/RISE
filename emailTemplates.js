...
function bookingReservedEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, first30, cleaning, pets, petFee, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const checkInDisplay = checkIn ? `${fmtLong(parseKey(checkIn))} at 3pm` : "";
  const checkOutDisplay = checkOut ? `${fmtLong(parseKey(checkOut))} at 11am` : "";

  const firstPaymentRows =
    detailRow("Check-in", checkInDisplay) +
    detailRow("Check-out", checkOutDisplay) +
