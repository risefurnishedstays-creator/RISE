...
function guestConfirmationEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const body = `
    ...
    ${calloutBox(`<strong>ACTION REQUIRED:</strong> <strong>Your booking isn't complete yet.</strong> Please sign your lease and upload a photo of your government-issued ID before your stay is confirmed.`, { tone: "warn" })}
...
function leaseSignedGuestEmail({ guestName, unitCode, unitName, checkIn, checkOut, confirmationCode, idUploadUrl }) {
  ...
  const body = `
    ...
    ${calloutBox(`<strong>One more thing to finish up:</strong> please upload a photo of the front of your government-issued ID. This is required to complete your booking.`, { tone: "warn" })}
...
function cancellationGuestEmail({ guestName, unitCode, confirmationCode, outcome }) {
  ...
  const body = `
    ...
    ${outcome.branch === "midstay" && outcome.midstayRule === 3 ? `<p style="font-size:13px; color:${BRAND.inkSoft};">Since the final payment for your stay had not yet been collected, it has now been charged to your card on file in full.</p>` : ""}
  `;
  return layout({
...
function liabilityInvoiceOwnerEmail({ booking, amount, invoice }) {
  const feeLabel = booking.midstayRule === 1 ? "Early termination fee" : "Final payment due";
  const body = `
