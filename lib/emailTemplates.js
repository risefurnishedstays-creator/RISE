...
function outcomeSummaryRow(outcome) {
  if (outcome.branch === "full-refund") {
    return `<tr><td style="padding:8px 0; font-weight:bold; width:170px;">Refund amount:</td><td style="padding:8px 0;">${money(outcome.refundAmount)} (rent: ${money(outcome.rentRefund)} + cleaning/pet fees: ${money(outcome.feeRefund)})</td></tr>`;
  }
  if (outcome.branch === "non-refundable") {
    return `<tr><td style="padding:8px 0; font-weight:bold; width:170px;">Refund amount:</td><td style="padding:8px 0;">${money(outcome.refundAmount)} — first month's rent is non-refundable inside 30 days of check-in, but the cleaning fee and pet fee(s) (${money(outcome.feeRefund)}) are refunded</td></tr>`;
  }
  // midstay
...
function cancellationOwnerEmail({ booking, outcome, refund, voidedInvoiceIds }) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      ${outcome.branch === "midstay" ? `<p style="color:#c0392b; font-size:13px;">Mid-stay cancellation — use the "Charge for liability period" action on the admin page to send the guest an invoice for the nights owed beyond what's already been charged.</p>` : ""}
...
function liabilityInvoiceOwnerEmail({ booking, amount, invoice }) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color:#2c3e50;">Liability Invoice Sent</h2>
      <p>A Stripe invoice was created and emailed to the guest for their mid-stay cancellation liability. They have not been auto-charged -- they need to pay via the hosted invoice link Stripe sent them.</p>
      <table style="width:100%; border-collapse:collapse;">
        <tr><td style="padding:8px 0; font-weight:bold; width:170px;">Confirmation #:</td><td style="padding:8px 0;">${escapeHtml(booking.confirmationCode)}</td></tr>
        <tr><td style="padding:8px 0; font-weight:bold;">Guest:</td><td style="padding:8px 0;">${escapeHtml(booking.guestName)} (${escapeHtml(booking.guestEmail)})</td></tr>
        <tr><td style="padding:8px 0; font-weight:bold;">Liable nights:</td><td style="padding:8px 0;">${escapeHtml(booking.liableNights)} (through ${escapeHtml(booking.liabilityEndDate)})</td></tr>
        <tr><td style="padding:8px 0; font-weight:bold;">Amount invoiced:</td><td style="padding:8px 0;">${money(amount)}</td></tr>
        <tr><td style="padding:8px 0; font-weight:bold;">Stripe invoice:</td><td style="padding:8px 0;">${escapeHtml(invoice.id)}</td></tr>
        <tr><td style="padding:8px 0; font-weight:bold;">Due date:</td><td style="padding:8px 0;">7 days from today</td></tr>
      </table>
      <p style="margin-top:16px;"><a href="${invoice.hosted_invoice_url}" style="color:#2c3e50;">View/share the hosted invoice link</a></p>
      <p style="color:#888; font-size:12px; margin-top:24px;">If the guest doesn't pay within the due window, you may want to follow up directly or pursue this per your Terms of Service.</p>
    </div>`;
}
