...
const { cancellationCutoffDates, parseKey, CONFIG } = require("./pricing");
...
function cancellationPolicyParagraph(checkInStr) {
  ...
  const earlyTermFee = CONFIG.MIN_NIGHTS * CONFIG.NIGHTLY;
  return `Cancel by <strong>${fmtLong(fullRefundCutoff)}</strong> and your first month's rent, cleaning fee, and any pet fees are refunded in full. ` +
    ...
    `After check-in, the cleaning fee and any pet fees become non-refundable, and an early termination fee of $${earlyTermFee.toLocaleString("en-US")} applies (or the remaining nightly rent through the original check-out date if fewer than ${CONFIG.MIN_NIGHTS} nights remain, whichever is less).`;
}
...
function guestConfirmationEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const body = `
    ...
    ${heading("Send your ID to finish booking" + (first ? ", " + first : ""))}
    ...
    ${calloutBox(`<strong>ACTION REQUIRED:</strong> <strong>Your booking isn't complete yet.</strong> Your lease is signed, but you still need to send a photo ID to <a href="mailto:risefurnishedstays@gmail.com" style="color:${BRAND.orange}; font-weight:bold;">risefurnishedstays@gmail.com</a> before your stay is confirmed.`, { tone: "warn" })}
    ...
    <p style="margin:24px 0 0; font-size:14px; line-height:1.6; color:${BRAND.inkSoft};">
      We'll send check-in details as your arrival date gets closer.
    </p>
  `;
...
function leaseSignedGuestEmail({ guestName, unitCode, unitName, checkIn, checkOut, confirmationCode }) {
  const first = (guestName || "").trim().split(" ")[0];
  const displayUnitName = unitName || unitDisplayName(unitCode);
  const cover = unitCoverPhoto(unitCode);
  ...
  const body = `
    ...
    <p style="margin:18px 0 0; font-size:14px; line-height:1.6; color:${BRAND.inkSoft};">
      One more thing to finish up: please send a photo of the front and back of your government-issued ID to <strong style="color:${BRAND.ink};">risefurnishedstays@gmail.com</strong>. This is required to complete your booking.
    </p>
    <p style="margin:18px 0 0; font-size:13px; line-height:1.6; color:${BRAND.inkSoft};">
      We'll follow up with check-in details and house rules closer to your arrival.
    </p>
  `;
