...
function bookingReservedEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, first30, cleaning, pets, petFee, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const firstPaymentRows =
    ...
    detailRowEmphasized("Due today", money(dueToday));

  // The Payment Schedule section only makes sense when there ARE future
  // installments to show -- priceParts() only populates `schedule` when
  // the stay is longer than 30 nights (see its own comment in
  // lib/pricing.js), so an empty schedule here means the entire stay was
  // covered by the single first payment above. Showing a whole second
  // "schedule" section that just restates the same one number as "Total
  // amount" added confusing duplication for these shorter stays, so it's
  // skipped entirely rather than rendered redundantly.
  const hasFutureInstallments = schedule && schedule.length > 0;
  const scheduleRowsHtml = hasFutureInstallments
    ? detailRow("Due today", money(dueToday)) +
...
// House rules are shared across the fourplex (see houseRulesForBooking()'s
// comment above), so unlike the check-in PDF this isn't keyed by unit --
// it's keyed by whether THIS booking actually has a pet, same logic
// houseRulesForBooking() already uses for the in-email rules list. Only
// Unit B is pet-friendly today, so in practice the pets version only ever
// applies there, but this is written generically rather than hardcoded to
// "B" in case that ever changes.
const HOUSE_RULES_PDF_NO_PETS = "https://www.risefurnishedstays.com/assets/house-rules-pdfs/house-rules.pdf";
const HOUSE_RULES_PDF_WITH_PETS = "https://www.risefurnishedstays.com/assets/house-rules-pdfs/house-rules-with-pets.pdf";
function houseRulesPdfAttachment(pets) {
  const url = pets > 0 ? HOUSE_RULES_PDF_WITH_PETS : HOUSE_RULES_PDF_NO_PETS;
  return { path: url, filename: pets > 0 ? "Cougar-Drive-House-Rules-Pets.pdf" : "Cougar-Drive-House-Rules.pdf" };
}
...
function bookingCompleteEmail({ guestName, unitCode, unitName, checkIn, checkOut, guests, pets, confirmationCode }) {
  const first = (guestName || "").trim().split(" ")[0];
  const displayUnitName = unitName || unitDisplayName(unitCode);
  const cover = unitCoverPhoto(unitCode);
...
function checkinInstructionsEmail({ guestName, unitCode, unitName, checkIn, checkOut, guests, pets, doorCode, guidebookUrl, confirmationCode }) {
  ...
  const body = `
    ...
    ${detailTable(
      ...
      detailRow("Wi-Fi network", "Cougar") +
      detailRow("Wi-Fi password", "fizzyphoenix695")
    )}
    <p style="margin:18px 0 0; font-size:15px; line-height:1.6; color:${BRAND.inkSoft};">
      We've attached step-by-step check-in instructions with photos to help you find the right door, along with our full house rules -- please give them a read before you arrive.
    </p>
...
function outcomeSummaryRow(outcome) {
  ...
  if (outcome.midstayRule === 2) {
    return detailRow("Early termination fee", "None") +
...
function contactConfirmationGuestEmail({ name, email, phone, message, unit, guests, pets }) {
  ...
  const body = `
    ...
    <p style="background:#fbeee5; padding:14px; border-radius:8px; white-space:pre-wrap; font-size:14px; line-height:1.5; color:${BRAND.ink}; border:1px solid ${BRAND.orange};">${escapeHtml(message)}</p>
    <p style="margin:22px 0 0; font-size:15px; line-height:1.6; color:${BRAND.inkSoft};">
...
function cancellationOwnerEmail({ booking, outcome, refund, voidedInvoiceIds, billedInvoice, billingError, noticeDate }) {
  const unitName = unitDisplayName(booking.unitCode);
  // noticeDate can arrive as either a full ISO timestamp (when defaulted
  // to "now" by the caller) or a plain "YYYY-MM-DD" (when backdated by an
  // admin processing a cancellation reported after the fact) -- fmtLong
  // + parseKey only handles the latter, so just take the date portion
  // either way rather than risking a parse error on the timestamp form.
  const noticeDateDisplay = noticeDate ? fmtLong(parseKey(noticeDate.slice(0, 10))) : "Not specified";
  const body = `
    ${detailTable(
      detailRow("Confirmation #", escapeHtml(booking.confirmationCode)) +
      detailRow("Guest", escapeHtml(booking.guestName) + " (" + escapeHtml(booking.guestEmail) + ")") +
      ...
      detailRow("Cancellation notice date", noticeDateDisplay) +
...
function checkoutInstructionsEmail({ guestName, unitCode, unitName, checkOut, confirmationCode }) {
  ...
  const body = `
    ...
    <p style="margin:0 0 18px; font-size:15px; line-height:1.6; color:${BRAND.inkSoft};">
      Please remember to bring all of your belongings with you when you leave -- our cleaning team will dispose of any items left behind in the unit.
    </p>
    <p style="margin:0 0 18px; font-size:15px; line-height:1.6; color:${BRAND.inkSoft};">
