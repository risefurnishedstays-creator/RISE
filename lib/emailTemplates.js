...
function bookingReservedEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const body = `
    ...
    ${detailTable(
      ...
      detailRow("Held on card (not yet charged)", money(dueToday)) +
      detailRowEmphasized("Full stay total", money(fullTotal))
    )}
    ${schedule && schedule.length ? `
      <p style="margin:22px 0 8px; font-weight:bold; font-size:14px; color:${BRAND.ink};">Installments after the first payment</p>
      ...
      <p style="color:${BRAND.inkSoft}; font-size:12.5px; margin-top:10px;">These are charged automatically to your card on their due dates, once the first payment itself has been charged. Stripe will email you a receipt each time.</p>
    ` : ""}
...
function guestConfirmationEmail({ guestName, unitCode, unitName, checkIn, checkOut, nights, dueToday, fullTotal, schedule, confirmationCode }) {
  ...
  const body = `
    ...
    ${specsLine ? `<p style="margin:0 0 18px; font-size:13.5px; color:${BRAND.inkSoft};">${escapeHtml(specsLine)}</p>` : ""}
    ${calloutBox(`<strong>One more thing:</strong> please sign your lease and upload your government-issued ID if you haven't already -- both are required to finish booking.`, { tone: "warn" })}
    <p style="margin:0 0 18px; font-size:14px; color:${BRAND.ink};"><strong>Confirmation code:</strong> ${escapeHtml(confirmationCode)}</p>
    ${detailTable(
      detailRow("Check-in", escapeHtml(checkIn)) +
      detailRow("Check-out", escapeHtml(checkOut)) +
    ...
    ${schedule && schedule.length ? `
      ...
      <p style="color:${BRAND.inkSoft}; font-size:12.5px; margin-top:10px;">Each installment is charged automatically to your card on file. Stripe will email you a receipt each time.</p>
    ` : ""}
    ${ctaButton("View " + displayUnitName, pageUrl)}
    <p style="margin:24px 0 0; font-size:14px; line-height:1.6; color:${BRAND.inkSoft};">
      Once your lease is signed and your ID is uploaded, we'll send you a final confirmation - no further action needed after that until your check-in details arrive about a week before arrival.
    </p>
  `;
...
function leaseSignedGuestEmail({ guestName, unitCode, unitName, checkIn, checkOut, confirmationCode, idUploadUrl }) {
  const first = (guestName || "").trim().split(" ")[0];
  const displayUnitName = unitName || unitDisplayName(unitCode);
  const cover = unitCoverPhoto(unitCode);
  ...
  const body = `
    ...
    ${calloutBox(`<strong>One more thing to finish up:</strong> please upload a photo of the front of your government-issued ID, or email it to <a href="mailto:${BRAND.supportEmail}" style="color:${BRAND.orange}; font-weight:bold;">${BRAND.supportEmail}</a>. This is required to complete your booking, and must be done before your check-in date.`, { tone: "warn" })}
...
function bookingCompleteEmail({ guestName, unitCode, unitName, checkIn, checkOut, confirmationCode }) {
  ...
  const body = `
    ...
    <p style="margin:0 0 18px; font-size:15px; line-height:1.6; color:${BRAND.inkSoft};">
      Your reservation for <strong style="color:${BRAND.ink};">${escapeHtml(displayUnitName)}</strong> (confirmation ${escapeHtml(confirmationCode)}) is fully confirmed - payment received, lease signed, and ID verified.
    </p>
    ${detailTable(
      detailRow("Check-in", escapeHtml(checkIn)) +
      detailRow("Check-out", escapeHtml(checkOut))
...
function contactConfirmationGuestEmail({ name, email, phone, message }) {
  ...
  const body = `
    ...
    <p style="margin:22px 0 0; font-size:13px; line-height:1.6; color:${BRAND.inkSoft};">
