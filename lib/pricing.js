...
//   stay already in progress    -> guest liable for next 30 nights from notice date
//
// today/noticeDate default to "now" but accept an override for testing.
function cancellationOutcome(checkInStr, checkOutStr, noticeDate) {
  const checkIn = parseKey(checkInStr);
  const checkOut = parseKey(checkOutStr);
  ...
  return {
    ...
    refundAmount: moreThan30DaysOut ? CONFIG.MIN_NIGHTS * CONFIG.NIGHTLY : 0,
    refundable: moreThan30DaysOut,
    liabilityEndDate: null,
    liableNights: 0,
    cancelFutureInstallments: true, // always cancel everything not yet charged
    daysUntilCheckIn,
    message: moreThan30DaysOut
      ? "Cancelled more than 30 days before check-in: first month's rent refunded in full."
      : "Cancelled 30 days or fewer before check-in: first month's rent is non-refundable.",
  };
}
