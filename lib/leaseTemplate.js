 ...
 */
function buildLeaseText({ guestName, unitCode, checkIn, checkOut, fullTotal, leaseDate }) {
...
(B) Tenant agrees to the following cancellation terms for this stay:

More than 30 days before check-in: If Tenant cancels more than 30 days before the scheduled check-in date, Tenant's first month's rent, cleaning fee, and any pet fee(s) will be refunded in full.

30 days or fewer before check-in: If Tenant cancels 30 days or fewer before the scheduled check-in date, the first month's rent is non-refundable. The cleaning fee and any pet fee(s) will be refunded in full.

After check-in: If Tenant terminates the stay early after check-in has occurred, the cleaning fee and any pet fee(s) become non-refundable, and Tenant must pay an early termination fee of $2,550, or the remaining nightly rent through the original check-out date if fewer than 30 nights remain, whichever is less.
