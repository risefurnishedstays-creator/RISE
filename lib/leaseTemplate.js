...
After check-in: If Tenant terminates the stay early on or after ${fmtLongDate(checkInDate)}, the cleaning fee and any pet fee(s) become non-refundable, and Tenant must pay an early termination fee of $${earlyTermFee.toLocaleString("en-US")}, or the remaining nightly rent through ${fmtLongDate(checkOutDate)} if fewer than ${CONFIG.MIN_NIGHTS} nights remain, whichever is less.
