...
(function () {
  ...
  // bookedRanges is rebuilt from U.booked; we refresh U.booked from the live
  // availability API (direct bookings + Airbnb/VRBO feeds) before first render.
  var bookedRanges = [];
  function rebuildBookedRanges() {
    bookedRanges = (U.booked || []).map(function (b) {
      // Feed-derived ranges use an EXCLUSIVE checkout date (checkout day is
      // free). This calendar treats 'to' as inclusive, so for ranges flagged
      // exclusive we step back one day. Direct-booking ranges from our own API
      // are also exclusive, so we treat all API ranges that way.
      var from = new Date(b.from + 'T00:00:00');
      var to = new Date(b.to + 'T00:00:00');
      // API/feed ranges set exclusiveEnd:true (checkout day free). Legacy
      // hardcoded ranges omit the flag and are treated as inclusive.
  ...
  /* ---------- init ---------- */
  // CHANGE THIS if your Vercel URL ever changes:
  var AVAIL_API = 'https://rise-eta-three.vercel.app';
  ...
  function recomputeFirstOpen() {
    ...
    calRef = new Date(firstOpen.getFullYear(), firstOpen.getMonth(), 1);
  }

  // Render immediately with whatever U.booked shipped in the page (fast paint),
  // then fetch live availability and re-render if it adds/changes anything.
  renderCal();
  updateSummary();

  var unitParam = (U.code || '').replace(/^Unit\s*/i, '').toUpperCase();
  if (unitParam) {
    fetch(AVAIL_API + '/api/availability?unit=' + encodeURIComponent(unitParam))
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (data) {
        if (!data || !data.booked) return;
        // Mark these as exclusive-end (checkout day free) and merge in.
        var live = data.booked.map(function (b) {
      ...
      .catch(function (e) { /* offline or API down: keep showing page data */ });
  }
})();
